...
const getTrips = async () => {
  ...
  const trips = tripData.slice(1).map((trip) => {
    return trip.split(",");
  });
  return { header, trips };
};
...
const getTripsByEndStations = async (arrStationId, date) => {
  ...
  return {
    header,
    trips: trips.filter((trip) => arrStationId.includes(trip[endStationIdIndex])
...
const getRecentTrips = async (arrStationId, date, rowsLimit = 20) => {
  ...
  const recentTrips = trips
    .sort((a, b) => a[endDateIndex] > b[endDateIndex] ? -1 : 1)
